...
void draw () {
  background(#F7FBFC);
  if (mode.equals("apprentissage") || mode.equals("questions")) {
...
// Vérifier si le département dans lequel on clique est le bon + update score
void mouseClicked() {

  // Si la question est sur les departements
  if (typeQuestion==0) {
...
void launcher() {
  ...
  //MODE COMPLETE LA CARTE
  if (mode.equals("complète")) {
    textFont(fontSousTitre);
    text("Tapez ici : ", width/2+130, 160);
    rect(width/2+250, 125, 525, 50);
    text(inputText, width/2+255, 155);
    textFont(grosTitre);
    text("COMPLÈTE LA CARTE", width/2+130, 100);
    if (jouer) {
      son_apprentissage.play();
      jouer=false;
    }
...
void keyPressed() {
  // Si la touche pressée est un caractère imprimable, on l'ajoute à inputText
  if (mode.equals("complète")) {
    if (key == ENTER) {
      for (int i = 0; i<departements.length; i++) {
        if (departements[i].nom_dept.toUpperCase().equals(inputText.toUpperCase())) {
          fill(departements[i].couleur_dept);
          shape(departements[i].pshape_dept, 30, 50);
        }
      }
      inputText="";
    }
    if (isPrintable(key) && inputText.length() < 32) {
      inputText += key; // Ajouter la touche à la chaîne de texte
    } else if (key == BACKSPACE && inputText.length() > 0) {
      // Si la touche est BACKSPACE, supprimer le dernier caractère
      inputText = inputText.substring(0, inputText.length() - 1);
    }
  }
}

// Fonction pour vérifier si un caractère est imprimable (lettres, chiffres, symboles)
boolean isPrintable(char c) {
  return Character.isLetterOrDigit(c) || isPunctuation(c);
}

// Fonction pour vérifier si le caractère est un signe de ponctuation ou un symbole courant
